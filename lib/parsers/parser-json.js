...
exports.extractHTTPPostMetrics = function(services, contentBuffer) {
	return new Promise((resolve, reject) => {
		try {
			...
			reject(new Error(`Invalid JSON content ([${err.name}] ${err.message})`));
		}
	})
		.then((json) => {
			...
			try {
				...
				throw new Error(`Invalid body JSON ([${err.name}] ${err.message})`);
			}

			if (!json.body || typeof json.body !== 'object') {
				throw new Error(`Invalid body JSON (non-object)`);
			}
