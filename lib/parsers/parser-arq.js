	...
	(services, result, path, text) => {
		if (path === 'html.body') {
			...
			if (match) {
				...
				}, `Found upload size: ${match[1]} ${match[2]} (${bytes})`);
	...
	(services, result, path, text) => {
		if (path === 'html.body') {
			...
			if (startMatch) {
				...
				}, `Found start time: ${result.startDate.toISOString()}`);
		...
		if (path === 'html.body') {
			...
			if (match) {
				...
				services.logger.debug({
					parsed: {
						text: match[1],
						date: result.endDate.toISOString(),
					},
				}, `Found end time: ${result.endDate.toISOString()}`);
				...
				if (match[4]) {
					...
					}, `Found error count from end time line: ${result.errorCount}`);
				}
		...
		.then((mail) => {
			...
			if (typeof mail.subject === 'string') {
				...
				if (subjectMatch) {
					...
					}, `Found error count in subject: ${result.errorCount}`);
				}
			}
			...
			const htmlParser = new HtmlParser({
				...
				onclosetag(name) {
					if (pathParts.length) {
						...
						path = pathParts.join('.');
						if (pop !== name) {
							services.logger.warn(`Popped ${name} HTML element but expected ${pop}`);
						}
					}
					else {
						services.logger.warn(`Found ${name} close HTML element but path is empty`);
					}
				},
			}, { decodeEntities: true });
